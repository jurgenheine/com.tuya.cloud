...
class TuayApi extends events.EventEmitter {
    ...
    setSessionData(result) {
        ...
        this.session.expireTime = new Date(Date.now().getTime() + (1000 * result.expires_in));
    ...
    async _check_access_token() {
        if (this.connectionError)
            throw this.lastMessage;
        if ((Date.now().getTime() - this.session.lastCall.getTime()) > AUTHREFRESHTIME) {
            if (this.session.accessToken === '' || this.session.refreshToken === '') {
                await this._get_access_token();
            }
            else if (this.session.expireTime <= new Date(Date.now().getTime() + 10000)) {
                //await this._get_access_token();
