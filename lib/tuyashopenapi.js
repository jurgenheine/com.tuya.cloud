...
class TuyaSHOpenAPI {
    ...
    async setHomeId() {
        let res = await this.get(`/v1.0/users/${this.tokenInfo.uid}/homes`);
        this.home_id = res.result[0].home_id;
    }
    ...
    async getScenes() {
        ...
        let res = await this.get(`/v1.0/homes/${this.home_id}/scenes`);
        return res.result;
    }

    async executeScene(scene_id) {
        if (this.home_id == null) {
            await this.setHomeId();
        }
    ...
    async getDevices() {
        ...
        if (devicesFunctions) {
            for (let i = 0; i < res.result.devices.length; i++) {
                devices.push(Object.assign({}, res.result.devices[i], devicesFunctions.find((j) => j.devices.find((k)=> k == res.result.devices[i].id))))
            }
        } else {
    ...
    async request(method, path, params = null, body = null) {
        ...
        try {
            ...
            this.log.lastConnectionError = e;
            this.log.log(e);
            return;
        }
